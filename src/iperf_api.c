...
iperf_exchange_parameters(struct iperf_test *test)
{
    ...
    if (test->role == 'c') {
        ...
        if (test->debug) {
            iperf_err(stderr, "iperf-exchange-parameters, setting read FD: %d.\n", s);
        }
            ...
            if (sp->sender == stream_must_be_sender) {
                ...
                if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
                    ...
                    if (test->json_output) {
                        ...
                        iperf_printf(test, report_bw_udp_format, sp->stream_id, mbuf, start_time, sender_time, ubuf, nbuf, 0.0, (int64_t) 0, (sender_packet_count - sender_omitted_packet_count), (double) 0, report_sender);
                ...
                if (test->sender_has_retransmits) {
                    ...
                    if (test->json_output)
                        ...
                        iperf_printf(test, report_sum_bw_retrans_format, mbuf,
                            start_time, sender_time, ubuf, nbuf, total_retransmits,
                            report_sender);
                } else {
                    ...
                    if (test->json_output)
                        ...
                        if (test->role == 's' && !stream_must_be_sender) {
                            iperf_printf(test, report_sum_bw_format, mbuf,
                                start_time, sender_time, ubuf, nbuf, report_sender);
                        }
                }
...
print_interval_results(struct iperf_test *test, struct iperf_stream *sp, cJSON *json_interval_streams)
{
	...
	if ((test->sender_has_retransmits == 1 || test->protocol->id == Psctp) && sp->sender) {
	    ...
	    if (test->json_output)
		cJSON_AddItemToArray(json_interval_streams, iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  omitted: %b sender: %b", (int64_t) sp->stream_id, (double) st, (double) et, (double) irp->interval_duration, (int64_t) irp->bytes_transferred, bandwidth * 8, irp->omitted, sp->sender));
	    else
	...
	if (sp->sender) {
	    if (test->json_output)
		cJSON_AddItemToArray(json_interval_streams, iperf_json_printf("id: %d socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  packets: %d  omitted: %b sender: %b", (int64_t)sp->stream_id, (int64_t) sp->stream_id, (double) st, (double) et, (double) irp->interval_duration, (int64_t) irp->bytes_transferred, bandwidth * 8, (int64_t) irp->interval_packet_count, irp->omitted, sp->sender));
	    else
	    ...
	    if (test->json_output)
		cJSON_AddItemToArray(json_interval_streams, iperf_json_printf("id: %d socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  jitter_ms: %f  lost_packets: %d  packets: %d  lost_percent: %f  omitted: %b sender: %b", (int64_t)sp->stream_id, (int64_t) sp->stream_id, (double) st, (double) et, (double) irp->interval_duration, (int64_t) irp->bytes_transferred, bandwidth * 8, (double) irp->jitter * 1000.0, (int64_t) irp->interval_cnt_error, (int64_t) irp->interval_packet_count, (double) lost_percent, irp->omitted, sp->sender));
	    else
